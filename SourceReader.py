...
def GetBackStarterID(listNowAndNextMemberNamesAndPattern, listMemberForArrange):
    ...
    while len(listIdsAndNames) == 0:
		...
		#todo: zip後應該是像這樣[1, (145, '詠馨')]，但是有個問題，就是萬一這個人本來沒有折扣，如果pattern是[0,1]那就ok。但是萬一他在listMemberForArrange加加減減的過程中變成有折扣，這時候如果pattern還是[0,1]，這就糟了。zip後可能會產生[0, (145, '詠馨')]....於是在下面的程式碼中，就變成最後回傳的intStartMemberId == 0....@@
        listZipped = zip(listNowAndNextMemberNamesAndPattern[i][1], listIdsAndNames)
        i += 1
    intStartMemberId = 0
